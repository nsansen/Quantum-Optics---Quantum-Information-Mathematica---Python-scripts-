function two_site_shrinking_algorithm_time_evolve_standalone(job_number)
% Script to build the flattened Liovillian and the diagonalise it:
% clear

if ischar(job_number), job_number = str2num(job_number); end;

save_prefix = 'two_site_spectrum_JCH_J_on_gamma_10_zoom_1_part_peak_';
save_name = [save_prefix num2str(job_number) '.mat'];

global spre_a spost_a spre_ad spost_ad spre_ada spost_ada spre_a_spost_ad spre_sm_spost_sp spre_spsm spost_spsm spre_adsm spost_adsm spre_asp spost_asp spre_photon_hopping spost_photon_hopping spre_trans_photon_hopping spost_trans_photon_hopping L_shrink
...
N0 = 4; % Initial number of photons in the left resonator
...
gamma_p = 0.00; % Cavity loss rate
gamma_a = 0;    % Atom loss rate

Omega = 0;   % Cavity driving strength

g_list = linspace(0, 2.5, 11)*g_c;

% Time rang efor integration:
...
y00_L = ad*ad*ad*ad*sm*y00_blank;
...
rho00_shrink = M_shrink_perm.'*reshape(rho00, [D_H^4 1]);
...
for loop_var = 1:length(var_list)
    ...
    for loop_t = 1:length(t)-1
        ...
        [TS, YS] = ode45(@masterkernel, t_int, rho0);
        
        rho_shrink_flat_t = YS(end,:).';
        rho0 = YS(end,:).';
...
save(save_name, 'num_1_store', 'ee_1_store', 'numsq_1_store', 'g2_ss_store', 'var_total_num_ss_store', 'var_list')
...
drho_flat_shrink = L_shrink*rho_flat_shrink;

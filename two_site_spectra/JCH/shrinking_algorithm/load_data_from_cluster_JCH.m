...
file_prefix = 'two_site_spectrum_JCH_increasing_delta_11';
...
for loop = 1:numjobs
    ...
    if data_loaded == 1
        ...
        if numjobs_loaded == 1
            ...
            ad = kron(speye(2), spdiags(sqrt([1:n_max].'), -1, n_max+1, n_max+1));
...
%     figure(figure_to_use)
    ...
    plot(-data.delta_omega_d_list/(data.g), log10(real(g2_plot)),'g','LineWidth',2)
    plot(-data.delta_omega_d_list/(data.g), log10(num_ss_store(loop,:)),'k','LineWidth',2)

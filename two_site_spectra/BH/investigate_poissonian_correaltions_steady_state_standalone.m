function investigate_poissonian_correaltions_steady_state_standalone(job_number)
% Script to find the steady state of a coherently driven linear system and
% investigate correlations between the resonators

if ischar(job_number), job_number = str2num(job_number); end;

save_prefix = 'lorentzian_investigation_data';
...
for loop = 1:length(var_list)
    ...
    delta_omega_d = delta_omega_d_list(loop);
...
save(save_name, 'n_max', 'M', 'gamma', 'Omega', 'delta_omega_d_list', 'J', 'U', 'rho_store')

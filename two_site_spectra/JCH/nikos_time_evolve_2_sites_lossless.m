function nikos_time_evolve_2_sites_lossless(job_number)
...
g_list = linspace(0, 2.5, 101)*g_c;

% Time rang efor integration:
t = linspace(0, 20*J, 1001);
...
%     H = jch_hamiltonian_multi_site_pbc(M, n_max, delta_omega_d, delta_omega_0, g, J, Omega);
    ...
    for loop_t = 1:length(t)-1
        ...
        [TS, YS] = ode45(@unitarykernel, t_int, y0);
        
        y0 = YS(end,:).';
        
        y_t = y0;
        
        num_t_store(loop, loop_t) = y_t'*num_L*y_t;
        numsq_t_store(loop, loop_t) = y_t'*num_L*num_L*y_t;
        num_R_t_store(loop, loop_t) = y_t'*num_R*num_R*y_t;
        densdens_cross_t_store(loop, loop_t) = y_t'*num_R*num_L*y_t;
        
    end
    
end

save(save_name, 'n_max', 'M', 'Omega', 'J', 'g', 'num_t_store', 'numsq_t_store')
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function drho_flat_shrink = unitarykernel(t,y)
% ODE kernel of the spins MF master equation

global H

drho_flat_shrink = -1i*H*y;
